...
* **Defensive C++ Memory & Thread Safety for Parallel Computing:**
  ...
  * **The Armadillo Subview Reference Trap:** When passing Armadillo subviews (e.g., `X.subvec(...)` or `X.col(...)`) to helper functions like `xi_coefficient` within intense Monte Carlo loops, **NEVER** use `const arma::vec&` (pass-by-reference). Armadillo dynamically generates temporary objects for subviews. In high-concurrency parallel environments (`multisession`), the rapid creation and destruction of these temporaries leads to dangling pointers, memory access violations, and immediate worker segmentation faults. You **MUST** use pass-by-value (`arma::vec`) for these functional inputs to guarantee a safe, thread-local memory copy.
  * **Strict In-Place Memory Recycling:** When generating surrogate data, **NEVER** allocate massive upfront matrices or 3D arrays (e.g., `arma::cube` of size N x p x n_surr). This will instantly trigger OS-level OOM-killers during parallel executions. You **MUST** strictly adhere to in-place memory recycling (e.g., allocating a single `arma::vec` or `arma::mat` outside the loop and overwriting it iteratively via internal worker functions like `generate_single_iaaft_worker`).
